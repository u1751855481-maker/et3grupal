...
class UIManager {
  ...
  refreshAction(action, payload = null) {
    ...
    let elementosFile = Array.from(
      document.querySelectorAll('.form-group input[type="file"]'),
    );

    if (!["ADD", "SEARCH"].includes(action)) {
      //si es un add o un search no tienen un antiguo file guardado, entonces no ponemos el enlace
      elementosFile.forEach((element) => {
        ...
        if (inputAntiguo && inputAntiguo.value) {
          // puede no existir en la estructura el antiguo file, por eso revisamos
    ...
    if (action !== "EDIT") {
      // si es EDIT deben existir los 2, xxxx_nuevo_yyyy xxxx_yyyy (siendo posible xxxx no existir)
      ...
      if (["ADD", "SEARCH"].includes(action)) {
        ...
        elementosFile.forEach((element) => {
          ...
          if (inputAntiguo) {
            ...
            alert(
              "Error: No se encontró el input text de archivo con ID: " +
                idAntiguo,
            );
          }
        });
      ...
      if (action !== "ADD") {
        // eliminar xxxx_nuevo_yyyy en acciones que no sean un EDIT (revisado antes) o un ADD
