class dom_table {
  ...
  showData(idcomponente, datatoshow, marked = null, hideable = false) {
    ...
    if (hideable) {
      const back_button =
        "<img src='./iconos/BACK.png' onclick=\"entidad.dom.hide_element('" +
        idcomponente +
        "');\" />";
        ...
        case "INPUT":
          switch (campos[i].type) {
            ...
            case "submit":
              break;
            case "checkbox":
              this.rellenarvalorcheckbox(
                campos[i].name,
                parametros[campos[i].name],
              );
              break;
            case "radio":
              this.rellenarvalorradio(
                campos[i].name,
                parametros[campos[i].name],
              );
              break;
          ...
          break;
        case "TEXTAREA":
          document.getElementById(campos[i].id).innerHTML =
            parametros[campos[i].id];
          break;
        case "SELECT":
  ...
  colocarvalidaciones(idform, accion) {
    ...
    for (let i = 0; i < campos.length; i++) {
      if (campos[i].id !== "") {
        if (
          document.getElementById(campos[i].id).tagName == "INPUT" &&
        ...
        if (document.getElementById(campos[i].id).type == "submit") {
          ...
          const validationTarget =
            campos[i].getAttribute("data-attribute-name") || campos[i].id;
          document
            .getElementById(campos[i].id)
            .setAttribute(
              evento,
              "entidad." +
                accion +
