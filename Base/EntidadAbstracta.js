...
class EntidadAbstracta {
  constructor(esTest) {
    ...
    if (esTest) {
      ...
      document
        .getElementById("text_title_page")
        .classList.add("text_titulo_page_" + this.nombreentidad);
      document
        .getElementById("text_title_page")
        .setAttribute("onclick", "entidad = new persona();");
  ...
  cargar_formulario() {
    if (eval(this.manual_form_creation)) {
      ...
      if (eval(this.cargar_formulario_dinamico)) {
        ...
        const message =
          langManager?.getText?.("error.form.not.found") ||
  ...
  async EDIT() {
    await this.access_functions
      .peticionBackGeneral("form_iu", this.nombreentidad, "EDIT")
      .then((respuesta) => {
        if (respuesta["ok"]) {
          //limpiar el formulario
  ...
  async ADD() {
    await this.access_functions
      .peticionBackGeneral("form_iu", this.nombreentidad, "ADD")
      .then((respuesta) => {
        if (respuesta["ok"]) {
          //limpiar el formulario
          document.getElementById("contenedor_IU_form").innerHTML = "";
  ...
  async DELETE() {
    await this.access_functions
      .peticionBackGeneral("form_iu", this.nombreentidad, "DELETE")
      .then((respuesta) => {
        if (respuesta["ok"]) {
          //limpiar el formulario
          document.getElementById("contenedor_IU_form").innerHTML = "";
        ...
        for (var clave in misdatos[i]) {
          for (var posicion in mostrarespeciales) {
            if (clave == mostrarespeciales[posicion]) {
              misdatos[i][clave] = this.mostrarcambioatributo(
                clave,
                misdatos[i][clave],
              );
            }
